var app =  new Vue({
  ...
  methods: {
    changeSiteCollection: function(siteCollection){
        ...
        this.messages.push({date: new Date(), verb: this.actions.Starting, text: 'Fetching Users and Groups', target: this.siteCollection.title, url: this.siteCollection.url, type: 'warning'});
        this.isLoading = true;

        (function(that){
          setTimeout(function(){
            that.messages.push({date: new Date(), verb: that.actions.Finished, text:  'Fetching Users and Groups', target: that.siteCollection.title, url: that.siteCollection.url, type: 'info'});
